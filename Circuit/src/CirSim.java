...
MouseMotionListener, MouseListener, ItemListener, KeyListener {
	...
	public void init() {
		...
		inputMenu.add(getClassCheckItem("Add Speaker Output", "SpeakerElm"));
	...
	void runCircuit() {
		...
		SpeakerElm theSpeaker = null;

		for (int ii = 0; ii != elmList.size(); ii++) {
			CircuitElm ce = getElm(ii);
			if (ce instanceof SpeakerElm) {
				theSpeaker = (SpeakerElm) ce;
			}
		}
		for (iter = 1; ; iter++) {
			int i, j, k, subiter;
