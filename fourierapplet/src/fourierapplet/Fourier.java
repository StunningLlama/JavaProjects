...
class FourierCanvas extends Canvas {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6189545278251682511L;
	FourierFrame pg;
...
public class Fourier extends Applet implements ComponentListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3710001738532204794L;
...
class FourierFrame extends Frame
  implements ComponentListener, ActionListener, AdjustmentListener,
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5868535615872937698L;
    ...
    Button amplifyButton;
    Button unamplifyButton;
	...
	/**
		 * 
		 */
		private static final long serialVersionUID = -4043469112945595430L;
    ...
    FourierCanvas cv;
    boolean java2;
    Hashtable<String, String> showTable;
	...
	try {
	    ...
	    if (show != null) {
		showTable = new Hashtable<String, String>(10);
		StringTokenizer st =
	...
	lowPassButton = doButton("Low-Pass Filter");
	amplifyButton = doButton("Amplify");
	unamplifyButton = doButton("De amplify");
	normalizeButton = doButton("Normalize");
	...
	for (i = 0; i != terms; i++)
		if (magcoef[i]*(i+1) > maximum) {
			maximum = magcoef[i]*(i+1);
		}*/
	for (i = 0; i != terms; i++)
		magcoef[i] *= (double)(i) / terms;
	doSetFunc();
    }
    ...
    /*	double maximum = 0;
    	...
    	for (i = 0; i != terms; i++)
    		if (magcoef[i]/(double)(i+1) > maximum) {
    			maximum = magcoef[i]/(double)(i+1);
    		}
    	if (maximum < 100000.0)*/
    	magcoef[0] = 0.0;
    	for (i = 1; i != terms; i++)
